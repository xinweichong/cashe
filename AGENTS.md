...
- NL confirmation buttons carry a random 32-hex draft token (`nl_confirm:<token>`, `nl_edit:<token>`, `nl_cancel:<token>`). A pending draft binds that token to its initiating username/chat. Stale, bare legacy, wrong-owner, and expired cards cannot save or clear a newer draft. `Storage.confirm_telegram_draft` atomically commits the request receipt, manual evidence, transaction, and requested capture-time trip job; generated FX is excluded from its submitted-field fingerprint. New source IDs use `telegram-nl-<token>`. Accepted/canceled/manual-edit drafts are cleared before awaiting Telegram replies, and only if still current. Validation failures retain matching Edit/Cancel actions. Unconfirmed drafts remain in memory and expire on restart; durable raw NL-message receipt/redelivery and draft persistence remain pending. Core confirmation dedup survives in per-user request records.
