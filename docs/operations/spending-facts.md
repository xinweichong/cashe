...
Storage is unchanged by these endpoints. Audited integer-money conversion, settlement precedence, explicit FX provenance, migration of legacy report consumers, and forecast inputs remain open in the plan. A new opt-in Home screen now consumes these facts (see [the audit](../plans/2026-09-09-cashe-next-level-audit.md) for its current, partial scope) — this doc's endpoints are not the Home UI itself.
