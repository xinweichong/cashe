...
## 2026-09-08 continuation — durable full-merchant recurring suggestions

Replayable legacy acceptance was committed as `38be6a9`; this continuation started with a clean working tree.

- Migration 10 adds durable recurring suggestions containing the full merchant, frequency, observed average, destination chat, pending/resolved state, and accepted schedule link. Identical pending fields reuse the record; no old buttons are backfilled.
- New Telegram buttons use opaque IDs below the 64-byte callback limit. The notifier selects the target user's Storage, commits the record off the event loop before sending, and retains the send Future/outbox acknowledgement contract. Long Unicode and delimiter-containing merchants survive acceptance intact.
- Accept/dismiss resolve only within the user's Storage and original chat. Both actions survive restart and are replayable; competing resolutions conflict. Acceptance atomically creates/reuses a schedule and its confirmation. Accepted links survive edits/deletion, preserving same-button protection against resurrection. The shared helper also retains migration 9 legacy behavior.
- Dismissal handles one suggestion without suppressing later detected patterns. Identical pending retries can send the same ID more than once; notifications after resolution may create a new record. Complete event identity, notification suppression controls, browser suggestion review, and observed-average money semantics remain separate work.
