...
**Follow-up (not done):**
- `OverviewPage` still has three v1 pieces with no v2 contract to move to: by-category trend, balance (income/expenses), goals. A balance/goal/category-trend typed contract would need to be built first — not done this pass.
- Analytics-tab: comparison, velocity, top-merchants(+trend), and anomalies/alerts are done; only the health-score typed contract remains.
