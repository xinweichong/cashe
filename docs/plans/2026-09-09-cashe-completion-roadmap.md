...
`OverviewPage` itself is still entirely on v1 — it's the large, information-dense page (multiple charts) the roadmap flagged as needing its own larger verification pass.

**Follow-up (not done):**
- Frontend migration of `OverviewPage` to the new v2 contracts — its chart data especially is built around v1's float shapes; converting it needs the same live-browser verification discipline used for the two slices above, just at much larger scale (multiple charts, budget/goal panels).
