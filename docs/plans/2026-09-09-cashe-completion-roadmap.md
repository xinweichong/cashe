...
**Follow-up (not done):**
- Shared trend, merchant/category contribution, trip, and forecast-input *typed contracts* don't exist — the money-correctness bug is fixed, but Overview/Analytics/Merchants still return ad hoc untyped dicts from `analytics.py`/`storage.py` rather than a shared v2 contract layer like transactions got in R03. Building that (new Pydantic contracts, new v2 routes, frontend migration) is a separate, larger task from the bug fix just done.
