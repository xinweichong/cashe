...
**Follow-up (not done):**
- `OverviewPage` has one remaining v1 piece: Goals Summary. Building its v2 contract is complicated by `get_goal_progress`'s monthly-rate formula being explicitly R13 scope (average-of-last-3-contributions, not dated/elapsed-window) — a contract could still wrap the existing formula as-is (same "adapter over current behavior, not a fix" pattern used elsewhere), but wasn't started this pass.
