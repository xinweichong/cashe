...
FinancePage's trip-row summary (`TripRow` in `FinancePage.tsx`) is the first frontend consumer, switched to `api.getTripSummaryV2`/`TripSummaryV2` — a deliberately small, low-risk first slice (that component only ever rendered `total_sgd`/`daily_average_sgd`/`transaction_count`, none of the richer breakdown fields). Verified end-to-end with a scripted Playwright session against a live backend, not just unit tests: created a transaction and trip through the real API, enlisted it, expanded the row, confirmed both the network call (`GET /api/v2/trips/{id}/summary -> 200`) and the rendered text matched. Overview/budget frontend consumers are still on v1.

**Follow-up (not done):**
- Frontend migration of `OverviewPage` and the budget-progress UI to the new v2 contracts — both are large, information-dense pages built around v1's float shapes (chart data especially, for Overview); converting them needs the same live-browser verification discipline used for the trip-row slice, just at larger scale.
