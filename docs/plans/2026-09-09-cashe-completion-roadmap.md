...
- `FinancePage`'s goal management panel (create/edit/contribute forms, using `api.getGoals`/v1 mutations) — not migrated; a `GoalProgress` v2 contract now exists for it to build on, but the editable-form conversion (pre-filling `target_amount` inputs from Money, etc.) wasn't attempted this pass.
