...
Remaining R06 sub-projects not yet started: duplicate detection/merge and merchant aliases + rule-impact preview — both assessed as Architectural (no existing subsystem to build on) rather than Bounded, so each needs the full brainstorming process (approaches, written spec, `writing-plans`) before implementation, unlike the four sub-projects closed so far.
