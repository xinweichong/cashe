...
**Follow-up (not done):**
- `OverviewPage` still has three v1 pieces with no v2 contract to move to: by-category trend, balance (income/expenses), and Goals Summary. (Its health score card was also still on v1 as of the last update — corrected below, now migrated.) A balance/goal/category-trend typed contract would need to be built first — not done this pass.
