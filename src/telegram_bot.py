...
class TelegramBotService:
    ...
    def _read_ai_narrative(self, storage, content_key: str, generated_at_key: str) -> Optional[str]:
        """Return today's cached LLM narrative for content_key, or None if
        missing, stale (not generated by this morning's job), or disabled."""
        generated_at = storage.get_setting(generated_at_key, "") or ""
        if not generated_at:
            return None
        try:
            ...
            return None
        if generated_date != self._local_now().date():
            return None
        raw = storage.get_setting(content_key, "") or ""
        if not raw:
            return None
        try:
    ...
    async def _delete_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        msg = (
            f"Delete this? It's gone for good.\n\n"
        ...
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton("Delete", callback_data=f"confirm_delete_{tx_id}"),
    ...
    async def _yesterday(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        narrative = self._read_ai_narrative(ctx.storage, "llm_daily_insight_content", "llm_daily_insight_generated_at")
        if narrative:
            text += f"\n\n{narrative}"
    ...
    async def _week(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if ctx is None:
            return
        text = self._format_spending_period(ctx.storage, "week")
        narrative = self._read_ai_narrative(ctx.storage, "llm_weekly_insight_content", "llm_weekly_insight_generated_at")
        if narrative:
            text += f"\n\n{narrative}"
        await self._send_long_message(update, text)
    ...
    async def _month(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if ctx is None:
            return
        text = self._format_spending_period(ctx.storage, "month")
        narrative = self._read_ai_narrative(ctx.storage, "llm_insight_content", "llm_insight_generated_at")
        if narrative:
            text += f"\n\n{narrative}"
        await self._send_long_message(update, text)
    ...
    async def _record_expense(self, ctx, update, context, parsed: dict, *, command: str, source: str,
        ...
        if tx_id is None:
            return
        lines = [f"Captured. *{self._escape_md(parsed['merchant'])}* · ${parsed['amount']:.2f}"]
        if conversion_line:
    ...
    async def _recategorize(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if len(context.args) >= 2:
            ...
            if new_category not in valid_categories:
                await update.message.reply_text(
                    f"Unknown category. /edit {tx_id} to pick from the list."
                )
                return
    ...
    async def _insights(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not summary["by_category"]:
            await update.message.reply_text("Nothing captured this month.")
            return
    ...
    async def _subscriptions(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not active_subs:
            await update.message.reply_text(
                "No subscriptions tracked. Add one via the app.",
            )
            return
    ...
    async def _handle_nl_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if action == "nl_confirm":
            ...
            msg = f"Captured. *{self._escape_md(pending['merchant'])}* · ${float(pending['amount']):.2f}"
    ...
    async def _handle_sub_suggest_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if data.startswith(("sub_suggest_accept:", "sub_suggest_dismiss:")):
            ...
            try:
                ...
                await query.edit_message_text("Couldn't finish this. Tap again or review subscriptions in the app.")
            return
        ...
        if data.startswith("sub_suggest_add|"):
            parts = data.split("|", 2)
            if len(parts) < 3:
                await query.edit_message_text("Couldn't parse this. Add it manually via the app.")
                return
        ...
        try:
            ...
            try:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text="Didn't go through. Try the command directly instead.",
                )
            except Exception:
                pass
    ...
    async def _send_daily_digest(self, chat_id: Optional[int] = None, storage=None) -> None:
        """Send a compact glance at yesterday + month-to-date, plus the
        cached AI read of the month if one was generated this morning."""
        _chat_id = chat_id if chat_id is not None else self.chat_id
        _storage = storage if storage is not None else self.storage
        if not _chat_id:
        ...
        with _storage.reconciliation_lock():
            day_facts = _storage.get_day_spending_facts(yesterday, self.timezone)
            month_facts = _storage.get_month_spending_facts(today, self.timezone)
        day_current = day_facts["current"]
        month_current = month_facts["current"]

        lines = [
            "*Morning Digest*",
            "",
            f"Yesterday: {_sgd_text(day_current['spending'])} · {day_current['status']}",
            f"Month to date: {_sgd_text(month_current['spending'])} "
            f"({month_current['start']} to {month_current['end']}) · {month_current['status']}",
        ]
        if month_current["recorded_net_flow"] is not None:
            lines.append(f"Net flow: {_sgd_text(month_current['recorded_net_flow'])}")

        narrative = self._read_ai_narrative(_storage, "llm_insight_content", "llm_insight_generated_at")
        if narrative:
            lines += ["", narrative]
    ...
    async def _forcepoll(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if count == 0:
                await update.message.reply_text("Nothing new captured.")
            else:
                await update.message.reply_text(f"{count} new transaction(s) captured.")
        except Exception as e:
            logger.warning("Force poll failed: %s", e)
            await update.message.reply_text("Poll failed.")
