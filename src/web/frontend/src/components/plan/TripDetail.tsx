...
export function TripDetail({ tripId, onClose }: { tripId: number; onClose: () => void }) {
  ...
  if (!trip) {
    return <DetailLoading title="Trip" onClose={onClose} />;
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <DetailHeader
        title={trip.name}
        subtitle={<p className="text-xs text-muted mt-0.5">{trip.destination ? `${trip.destination} · ` : ''}{dateLabel}</p>}
        onClose={onClose}
      />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {confirmDelete ? (
          <ConfirmDestructive
            message={`Delete trip "${trip.name}"? Its transactions stay in Activity; only the trip grouping is removed.`}
